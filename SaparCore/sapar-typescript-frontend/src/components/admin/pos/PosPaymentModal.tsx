...
export const PosPaymentModal: React.FC<Props> = ({
  ...
  // Dynamic UzQR config & Polling
  const [uzqrRef] = useState<string>(() => `POS-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`);
  const [uzqrConfig, setUzqrConfig] = useState<{ merchantId: string; terminalId: string; bankName: string } | null>(null);
  const [uzqrPaid, setUzqrPaid] = useState<boolean>(false);

  useEffect(() => {
    if (!isOpen) return;
    const token = localStorage.getItem('token');
    const apiBase = (window as any)?.__API_BASE__ || '/api';
    fetch(`${apiBase}/admin/payment-gateways/settings`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
      .then((d) => {
        if (d?.data?.uzqr) {
          setUzqrConfig(d.data.uzqr);
        }
      })
      .catch(() => {});
  }, [isOpen]);
  ...
  useEffect(() => {
    if (!isOpen || activeTab !== 'qr' || uzqrPaid) return;
    const token = localStorage.getItem('token');
    const apiBase = (window as any)?.__API_BASE__ || '/api';

    const interval = setInterval(() => {
      fetch(`${apiBase}/admin/payments/uzqr/status/${uzqrRef}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then((r) => r.json())
        .then((res) => {
  ...
  }, [isOpen, activeTab, uzqrRef, uzqrPaid, onCompletePayment, totalAmount]);
          ...
          {activeTab === 'qr' && (
            <div className="p-6 rounded-3xl bg-gradient-to-b from-teal-50/70 to-white border border-teal-200 text-center space-y-4 shadow-xs">
              ...
              <div className="inline-block p-4 bg-white rounded-3xl border-2 border-teal-600 shadow-md">
                <QRCodeSVG
                  value={`uzqr://pay?m=${encodeURIComponent(uzqrConfig?.merchantId || 'UZQR-MERCHANT-7788')}&t=${encodeURIComponent(uzqrConfig?.terminalId || 'TERM-001')}&a=${totalAmount}&ref=${encodeURIComponent(uzqrRef)}&cur=860`}
                  size={144}
                  level="M"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                {uzqrPaid ? (
                  <div className="inline-flex items-center gap-1.5 text-xs text-emerald-800 font-bold bg-emerald-100 px-3 py-1 rounded-full border border-emerald-300 animate-pulse">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                    Toʻlov muvaffaqiyatli qabul qilindi! Yakunlanmoqda...
                  </div>
                ) : (
                  <div className="inline-flex items-center gap-1.5 text-xs text-teal-800 font-semibold bg-teal-50 px-3 py-1 rounded-full border border-teal-200">
                    ...
                    Mijoz toʻlashi kutilmoqda ({uzqrConfig?.bankName || 'Ipak Yoʻli Bank'})...
                  </div>
                )}
              </div>
            </div>
