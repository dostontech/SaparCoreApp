...
export const SaparSidebar: React.FC<SaparSidebarProps> = ({ isOpen, onClose, onToggle }) => {
  ...
  // Custom menu order persisted in localStorage
  const [customMenuOrder, setCustomMenuOrder] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('sapar_sidebar_menu_order');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
  ...
  // Drag and drop states for vertical menu reordering
  ...
  const orderedMenuItems: MenuItem[] = useMemo(() => {
    if (!customMenuOrder || customMenuOrder.length === 0) return menuItems;
    const itemMap = new Map(menuItems.map((item) => [item.id, item]));
    const ordered: MenuItem[] = [];

    customMenuOrder.forEach((id) => {
  ...
  const handleDragOver = (e: React.DragEvent, id: string) => {
    ...
    if (dragOverItemId !== id || dropPosition !== position) {
  ...
  const handleDrop = (e: React.DragEvent, targetId: string) => {
    ...
    const currentIds = orderedMenuItems.map((i) => i.id);
    ...
    if (targetIndex !== -1) {
      ...
      setCustomMenuOrder(withoutSource);
      try {
        localStorage.setItem('sapar_sidebar_menu_order', JSON.stringify(withoutSource));
      } catch {
        // ignore
      }
      toast.success('Menyu tartibi yangilandi', { duration: 1500 });
    }
    resetDragState();
  };
  ...
  const handleResetMenuOrder = () => {
    setCustomMenuOrder([]);
    try {
      localStorage.removeItem('sapar_sidebar_menu_order');
    } catch {
      // ignore
    }
    toast.info('Menyu boshlangʻich tartibga qaytarildi', { duration: 1500 });
  };
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    if (!query) return orderedMenuItems;
  ...
  }, [location.pathname, location.search, orderedMenuItems]);
  ...
  const sidebarContent = (
    <aside className="w-64 bg-[#082026] text-slate-100 h-full flex flex-col select-none border-r border-[#13444D] transition-all duration-200 shrink-0 overflow-hidden font-sans shadow-lg">
      ...
      <div className="flex-1 overflow-y-auto py-2.5 px-2 space-y-0.5 custom-scrollbar text-xs">
        {/* Custom Order Active indicator & Reset Action */}
        {customMenuOrder.length > 0 && !searchQuery && (
          <div className="flex items-center justify-between px-2.5 py-1 mb-1.5 rounded-lg bg-[#0D3B46]/60 border border-[#13444D] text-[10px] text-slate-300 shadow-xs">
            <div className="flex items-center gap-1.5 text-[#02C39A] font-semibold">
              <GripVertical className="w-3 h-3" />
              <span>Mos tartib faol</span>
            </div>
            <button
              type="button"
              onClick={handleResetMenuOrder}
              className="flex items-center gap-1 text-slate-400 hover:text-white transition cursor-pointer hover:underline"
              title="Boshlangʻich tartibga qaytarish"
            >
            ...
            {filteredMenuItems.map((item) => {
              ...
              return (
                <div
                  ...
                  className={`group/drag relative transition-all duration-150 ${isItemDragged ? 'opacity-30 scale-[0.98]' : 'opacity-100'}`}
                >
                  {/* Drop indicator TOP */}
                  {isItemDragOver && dropPosition === 'top' && (
                    <div className="h-1 bg-[#02C39A] rounded-full mx-1 my-0.5 shadow-[0_0_8px_#02C39A] animate-pulse pointer-events-none transition-all" />
                  )}

                  {!hasChildren && item.to ? (
                    // Single link item (e.g. Bosh panel)
                    <div className="group relative flex items-center justify-between rounded-lg transition">
                      <NavLink
                        to={item.to}
                        end
                        className={({ isActive }) =>
                      ...
                      >
                        <div
                          className="opacity-0 group-hover/drag:opacity-50 hover:!opacity-100 transition cursor-grab active:cursor-grabbing text-slate-400 p-0.5 -ml-1 mr-0.5 shrink-0"
                          title="Surib oʻrnini oʻzgartirish (yuqoriga / pastga)"
                      ...
                      <button
                        ...
                        className={`absolute right-2 p-1 rounded-md transition-all ${isFavorite(item.to)
                          ? 'text-amber-400 opacity-100'
                          : 'text-slate-400 hover:text-amber-400 opacity-0 group-hover:opacity-100'
                          }`}
                        title={isFavorite(item.to) ? 'Sevimlilardan oʻchirish' : 'Sevimlilarga qoʻshish'}
                      >
                        <Star
                          className={`w-3.5 h-3.5 transition-transform duration-150 active:scale-125 ${isFavorite(item.to) ? 'fill-amber-400 text-amber-400' : ''
                            }`}
                        />
                      </button>
                    </div>
                    ...
                    <AccordionPrimitive.Item value={item.id} className="border-none space-y-0.5">
                  ...
                  {isItemDragOver && dropPosition === 'bottom' && (
                    <div className="h-1 bg-[#02C39A] rounded-full mx-1 my-0.5 shadow-[0_0_8px_#02C39A] animate-pulse pointer-events-none transition-all" />
                  )}
                </div>
              );
