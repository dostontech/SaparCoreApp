...
import { CirclePlusIcon, Edit, Trash2Icon, ShieldCheck } from "lucide-react";
...
const InvoiceList: React.FC = () => {
    ...
    const [signInvoice, setSignInvoice] = useState<Invoice | null>(null);
    ...
    const getTableActions = (item: Invoice): Action<Invoice>[] => {
        const actions: Array<Action<Invoice> & { hideWhen?: string[] }> = [
            {
                label: 'E-IMZO (E-Faktura)',
                icon: <ShieldCheck size={14} className="text-teal-600" />,
                primary: true,
                onClick: () => setSignInvoice(item),
                hideWhen: ['CANCELLED']
            },
            {
                label: 'Edit',
    ...
    return (
        <div className="space-y-4">
            ...
            {signInvoice && (
                <EimzoSignModal
                    isOpen={!!signInvoice}
                    onClose={() => setSignInvoice(null)}
                    invoiceId={signInvoice.id}
                    invoiceNumber={signInvoice.invoiceNumber}
                    customerName={signInvoice.billTo?.name}
                    totalAmount={signInvoice.TotalAmount}
                    onSuccess={() => {
                        toast.success('Hujjat E-IMZO bilan imzolandi va Didox orqali yuborildi');
                        fetchInvoices();
                    }}
                />
            )}
