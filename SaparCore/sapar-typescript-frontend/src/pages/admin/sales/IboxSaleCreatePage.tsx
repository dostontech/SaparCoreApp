  ...
  return (
    <div className="max-w-7xl mx-auto font-sans text-slate-800 pb-24 space-y-4 animate-fade-in text-xs">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-black text-slate-900">Sotuv (Yaratish)</h1>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate('/admin/pos')}
            className="bg-amber-500 hover:bg-amber-600 text-white font-bold border-none text-xs"
          >
            POS oyna
          </Button>
        </div>
      ...
      {/* Main Form Container (Exact white card from ibox_sale_create.png) */}
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-5">
        {/* Row 1: Mijoz, Sotuv sanasi, Sotuv raqami */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Mijoz*</label>
            <select
              value={customer}
              onChange={(e) => setCustomer(e.target.value)}
              className="w-full px-3 py-2 rounded-lg border border-slate-300 font-bold text-slate-800 bg-white focus:outline-none focus:border-blue-500"
            >
          ...
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Sotuv sanasi*</label>
            <div className="relative">
              <input
                type="text"
                value={saleDate}
                onChange={(e) => setSaleDate(e.target.value)}
                className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-300 font-mono text-slate-800 focus:outline-none focus:border-blue-500"
              />
          ...
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Sotuv raqami</label>
            <input
              type="text"
              value={saleNumber}
              onChange={(e) => setSaleNumber(e.target.value)}
              className="w-full px-3 py-2 rounded-lg border border-slate-300 font-mono text-slate-800 focus:outline-none focus:border-blue-500"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Sotuv kanali</label>
            <select
              value={salesChannel}
              onChange={(e) => setSalesChannel(e.target.value)}
              className="w-full px-3 py-2 rounded-lg border border-slate-300 text-slate-800 bg-white focus:outline-none focus:border-blue-500"
            >
          ...
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Masʼul shaxs</label>
            <div className="relative">
              <input
                type="text"
                value={responsiblePerson}
                onChange={(e) => setResponsiblePerson(e.target.value)}
                className="w-full pl-3 pr-8 py-2 rounded-lg border border-slate-300 font-bold text-slate-800 focus:outline-none focus:border-blue-500"
              />
              <button
                type="button"
          ...
          <div>
            <label className="block text-slate-500 font-semibold mb-1">Hujjat valyutasi*</label>
            <select
              value={currency}
        ...
        {/* Row 3: Ombor, Buttons (Mahsulotlarni tanlash, Excel dan yuklash), Prays list */}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div>
              <select
                ...
                className="px-3 py-2 rounded-lg border border-slate-300 font-bold text-slate-800 bg-white focus:outline-none focus:border-blue-500"
              >
            ...
            <Button
              ...
              className="bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold text-xs"
            >
            ...
            >
              <FileSpreadsheet className="w-3.5 h-3.5 mr-1 text-emerald-600" /> Excel dan yuklash
            </Button>
          </div>
        ...
        {/* 1-to-1 BLUE TABLE HEADER (Exact match to ibox_sale_create.png) */}
        <div className="overflow-x-auto rounded-lg border border-slate-200">
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-[#2563EB] text-white font-bold text-[11px] uppercase tracking-wider">
              <tr>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 pt-3">
          <div className="lg:col-span-8 space-y-3">
            <div>
              <label className="block text-slate-500 font-semibold mb-1">Izoh</label>
              <textarea
                rows={3}
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Buyurtmaga tegishli qoʻshimcha izohlar..."
                className="w-full px-3 py-2 rounded-lg border border-slate-300 text-slate-800 focus:outline-none focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-slate-500 font-semibold mb-1">Yetkazib berish manzili</label>
              <div className="relative">
                <input
                  type="text"
                  value={deliveryAddress}
                  onChange={(e) => setDeliveryAddress(e.target.value)}
                  className="w-full pl-9 pr-9 py-2 rounded-lg border border-slate-300 text-slate-800 focus:outline-none focus:border-blue-500"
                />
                <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
                <MapPin className="w-4 h-4 text-blue-600 absolute right-3 top-2.5 cursor-pointer" />
              </div>
            </div>
          </div>

          <div className="lg:col-span-4 flex flex-col justify-end space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div className="flex justify-between text-slate-600 items-center">
              ...
              <span className="font-mono text-slate-700 flex items-center gap-1">
                0 UZS <Edit2 className="w-3 h-3 text-blue-600 cursor-pointer" />
              </span>
            </div>
            <div className="border-t border-slate-200 pt-2 flex justify-between items-center">
              <span className="font-black text-slate-900 text-sm">Jami:</span>
              <span className="font-black text-blue-700 text-base font-mono">{format(subtotal)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-3 shadow-lg flex items-center justify-between z-30">
        <div className="flex items-center gap-3">
          <Button onClick={handleSaveSale} className="bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-xs">
            Saqlash <ChevronDown className="w-3.5 h-3.5 ml-1" />
          </Button>
          <Button variant="outline" onClick={() => navigate('/admin/invoices')} className="text-xs">
            Bekor qilish
          </Button>
        </div>

        <div className="flex items-center gap-5 text-xs">
          <span>Jami: <strong className="font-mono text-blue-700 text-sm">{format(subtotal)}</strong></span>
          <span>Miqdor: <strong className="font-mono text-slate-900">{totalQty}</strong></span>
        </div>
      </div>
    </div>
  );
