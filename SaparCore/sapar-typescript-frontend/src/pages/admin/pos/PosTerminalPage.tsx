...
export const PosTerminalPage: React.FC = () => {
  ...
  const handleAddToCartWithMarking = (prod: any, markingCode: string) => {
    const itemPrice = getCalculatedPrice(prod.price);
    posAudio.playScanBeep();
    setCart((prev) => [
      ...prev,
  ...
  const handleSearchKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      ...
      // Detect GS1 DataMatrix (Asl Belgisi): starts with (01) or 01 and length >= 20
      if (q.startsWith('(01)') || (q.startsWith('01') && q.length >= 24)) {
        try {
          const verifyRes = await axios.post(
            `${Constants.API_BASE_URL}/admin/marking/verify`,
            { code: q },
            { headers: { Authorization: `Bearer ${token}` } }
          );
          ...
          if (verifyRes.data?.blocked) {
            posAudio.playRemoveTone();
            toast.error(verifyRes.data.message || 'DIQQAT! Ushbu tovar yaroqlilik muddati oʻtgan yoki bloklangan!', {
              duration: 6000,
            });
            setSearchQuery('');
            return;
          ...
          const parsed = verifyRes.data?.data;
          const gtin = parsed?.gtin;
          const matched = products.find(
            (p) =>
              (gtin && (p.barcode?.includes(gtin) || gtin.includes(p.barcode || ''))) ||
              p.barcode === q ||
              p.sku === gtin
          ) || products[0];

          if (matched) {
            handleAddToCartWithMarking(matched, q);
            setSearchQuery('');
            toast.success(`🇺🇿 Asl Belgisi kodi qabul qilindi: ${parsed?.serialNumber || 'DataMatrix'}`);
            return;
          }
        } catch (err: any) {
          if (err.response?.data?.blocked) {
            posAudio.playRemoveTone();
            toast.error(err.response.data.message || 'DIQQAT! Vazirlar Mahkamasining 296-son qaroriga muvofiq ushbu tovar sotuvi taqiqlangan!', {
              duration: 7000,
            });
            setSearchQuery('');
            return;
                  ...
                  <div key={item.id} className="py-2.5 flex items-center justify-between gap-3 text-xs">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {(item as any).markingCode && (
                          <span className="text-[9px] bg-emerald-100 text-emerald-800 font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap">
                            🇺🇿 Asl Belgisi
                          </span>
                        )}
                      </div>
