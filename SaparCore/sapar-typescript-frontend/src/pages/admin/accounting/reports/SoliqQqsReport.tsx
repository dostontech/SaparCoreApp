...
export const SoliqQqsReport: React.FC = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'QQS hisobotini yuklashda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignAndSubmitSoliq = async () => {
    try {
      ...
      const payloadData = data || {
        taxType: "Qo'shilgan qiymat solig'i (QQS 12%)",
        formCode: '10006_29',
        period: `${from} — ${to}`,
        summary: { netVatPayableToBudget: 15000000 },
      };
      ...
      toast.error(err.response?.data?.message || 'Soliq.uz ga yuborishda xatolik yuz berdi');
    } finally {
