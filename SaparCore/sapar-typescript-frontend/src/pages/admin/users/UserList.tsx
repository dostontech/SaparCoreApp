...
const UserList: React.FC = () => {
    ...
    const tableActions = [
        ...(hasPermission(permissions, "roles-permissions", "edit") ? [{
    ...
    const tableHeaders = [
        '#',
        t("users.userName", "Foydalanuvchi"),
        t("common.phone", "Telefon"),
        t("users.role", "Roli"),
        t("users.createdOn", "Yaratilgan sana"),
        ...(tableActions.length > 0 ? [t("common.actions", "Amallar")] : [])
    ];
    ...
    return (
        <div className="space-y-4">
            <PageHeader title={t("users.title", "Foydalanuvchilar")}>
                {hasPermission(permissions, "roles-permissions", "create") &&
                    ...
                    >
                        {t("users.newUser", "Yangi Foydalanuvchi")}
                    </Button>
                }
            </PageHeader>
            ...
            <div className="flex justify-between items-center">
                ...
                >
                    {[10, 25, 50].map((num) => (
                        <option className="text-gray-950" key={num} value={num}>{num} ta / sahifa</option>
                    ))}
                </select>
            </div>
            {/* Table */}
            <Table headers={tableHeaders}>
                ...
                {!isLoading && !staffs.length &&
                    <tr>
                        <td colSpan={tableHeaders.length} className="text-center text-gray-950 py-2 font-semibold">{t("users.noUsersFound", "Hech qanday foydalanuvchi topilmadi")}</td>
                    </tr>
                }
            ...
            <DeleteConfirmationModal
                ...
                title={t("users.deleteTitle", "Foydalanuvchini oʻchirish")}
                message={t("users.deleteConfirmMessage", "Haqiqatan ham ushbu foydalanuvchini oʻchirmoqchimisiz? Bu amalni qaytarib boʻlmaydi.")}
            />
        </div>
    );
}
