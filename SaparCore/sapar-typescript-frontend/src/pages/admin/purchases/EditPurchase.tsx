...
const EditPurchase: React.FC = () => {
    ...
    // Sync default currency code once loaded (only if user hasn't yet set one)
    useEffect(() => {
        if (defaultCurrencyCode) {
            setPurchaseFormData(prev => {
                if (prev.currencyCode) return prev;
                return { ...prev, currencyCode: defaultCurrencyCode };
            });
        }
    }, [defaultCurrencyCode]);
    ...
    // ==========================================
    // 1. HANDLERS (Defined first for hoisting)
    // ==========================================
    ...
    // ==========================================
    // 2. FETCHERS (Defined before useEffects)
    // ==========================================
    ...
    const fetchPurchase = async () => {
        try {
            ...
            if (data) {
                // Contact-first: prefill the ContactPicker from the purchase's contactId
                // (falls back to vendor/billTo id for legacy records). Tax treatment comes
                // from the saved purchase, so we don't re-apply the contact default here.
                const preselectContactId = data.contactId ?? data.vendor?.id ?? data.billTo?.id ?? null;
                setSelectedContactId(preselectContactId);
    ...
    const validatePurchaseOrderData = () => {
        ...
        const lineFieldError = validateLineCustomFields(purchaseFormData.items, lineFields, docCreatedAt);
                ...
                <div className="max-w-7xl mx-auto space-y-4">
                    ...
                    <div className="w-full">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className="w-full">
                                <label htmlFor="po-id" className="block text-sm font-medium text-heading mb-1">
                                    Order ID
                                </label>
                                <input
                                    type="text"
                                    ...
                                    readOnly
                                    className="border bg-gray-100 border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-purple-600"
                                />
                            </div>
                            <div className="w-full">
                                <DateInput
