...
// Landing page that completes the whatsappcrm → sapar SSO handshake.
// 1. Reads the short-lived JWT from ?token=
// 2. Exchanges it for a sapar session token via the backend
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const { token: kanakkuToken, user } = res.data || {};
                if (!kanakkuToken || !user) {
                    throw new Error("Malformed SSO response.");
                }

                Cookies.set("authToken", kanakkuToken, { secure: window.location.protocol === "https:", sameSite: "Strict", expires: 7 });
