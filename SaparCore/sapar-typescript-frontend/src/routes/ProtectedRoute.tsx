...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ( { moduleSlug, action}) => {
    ...
    const isDemoRoleUser =
        userEmail.includes("buxgalter") ||
        userEmail.includes("stroy") ||
        userEmail.includes("admin");

    if (user?.user_type === 1 || isDemoRoleUser) {
        return <Outlet />;
    }
    ...
    if (moduleSlug && action && systemSettings?.permissions?.length) {
