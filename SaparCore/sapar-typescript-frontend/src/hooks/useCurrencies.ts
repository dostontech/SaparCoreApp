...
export function useCurrencies() {
    ...
    const resolveCurrency = useCallback(
        (code: string | undefined | null): CurrencyOption => {
            ...
            // last resort (Uzbekistan / Central Asia standard)
            return { id: 'uzs', code: 'UZS', symbol: "soʻm", name: "Oʻzbekiston soʻmi", status: true, isDefault: true };
        },
    ...
    const formatMoney = useCallback(
        (amount: number | string | null | undefined, code: string | undefined | null): string => {
            ...
            try {
                ...
                return `${n.toLocaleString('uz-UZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${resolved.symbol}`;
            }
        },
        [resolveCurrency]
    );
    ...
    const defaultCurrencyCode = systemSettings?.currency?.code ?? 'UZS';
