...
This guide covers what happens the first time you open Sapar after installation,
...
- Logo (uploaded file, stored in the `sapar-uploads` volume)
...
Sapar ships with six accounting country packs, each with a pre-configured
...
The `down-clean` target removes the `sapar-pg-data` and `sapar-uploads`
