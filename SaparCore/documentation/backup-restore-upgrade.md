...
| Volume | Contents |
|---|---|
| `sapar_sapar-pg-data` | The entire PostgreSQL database (all invoices, customers, settings, transactions) |
| `sapar_sapar-uploads` | Uploaded files: invoice and company logos, expense receipt attachments |
...
docker compose --env-file docker/.env -f docker/docker-compose.yml \
  exec -T postgres pg_dump -U sapar sapar \
  | gzip > sapar-$(date +%F).sql.gz
```
...
```bash
docker run --rm \
  -v sapar_sapar-uploads:/data \
  -v "$PWD":/backup \
  alpine \
  tar czf /backup/uploads-$(date +%F).tar.gz -C /data .
```

Note: the volume name is prefixed with the compose project name (`sapar_`).
...
```cron
0 2 * * * cd /path/to/sapar && \
  docker compose --env-file docker/.env -f docker/docker-compose.yml \
    exec -T postgres pg_dump -U sapar sapar \
    | gzip > /backups/sapar-$(date +\%F).sql.gz && \
  docker run --rm \
    -v sapar_sapar-uploads:/data \
    -v /backups:/backup \
...
Replace `/path/to/sapar` with your actual repo root and `/backups` with
...
```bash
gunzip -c sapar-2026-01-15.sql.gz \
  | docker compose --env-file docker/.env -f docker/docker-compose.yml \
    exec -T postgres psql -U sapar sapar
```
...
docker compose --env-file docker/.env -f docker/docker-compose.yml \
  exec postgres psql -U sapar -c "DROP DATABASE sapar;"
docker compose --env-file docker/.env -f docker/docker-compose.yml \
  exec postgres psql -U sapar -c "CREATE DATABASE sapar;"
...
cd sapar-typescript-backend && git pull && cd ..
cd sapar-typescript-frontend && git pull && cd ..
make up
```
...
# Expected: {"status":"ok","service":"sapar-api",...}
```
