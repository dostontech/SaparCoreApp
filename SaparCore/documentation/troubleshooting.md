...
**Symptom:** Logging in with `admin@demo.sapar.local` / `Demo123$` fails
...
```bash
docker compose --env-file docker/.env -f docker/docker-compose.yml \
  exec postgres psql -U sapar sapar \
  -c "UPDATE \"User\" SET email = 'admin@demo.sapar.local' WHERE email = 'admin@example.com';"
```
...
`sapar_sapar-uploads` Docker volume, mounted into the API container
...
docker inspect sapar-api-1 | grep -A5 '"Health"'
```
