...
This is the most important operational rule in Sapar:
...
| `DATABASE_URL` | Yes | — | PostgreSQL connection string. The hostname must be `postgres` (the compose service name). The password must match `POSTGRES_PASSWORD`. | `postgresql://sapar:yourpass@postgres:5432/sapar?schema=public` |
...
| `SMTP_FROM` | No | — | Sender address shown on outgoing mail | `Sapar <billing@example.com>` |
...
| Variable | Required | Default | Purpose | Example |
|---|---|---|---|---|
| `POSTGRES_USER` | Yes | `sapar` | Database username. Must match the user in `DATABASE_URL`. | `sapar` |
| `POSTGRES_PASSWORD` | Yes | — | Database password. **Change this.** Must match the password in `DATABASE_URL`. | `str0ng-unique-pass` |
| `POSTGRES_DB` | Yes | `sapar` | Database name. Must match the database in `DATABASE_URL`. | `sapar` |
...
|---|---|---|---|
| `postgres` | Yes | — | PostgreSQL 16. Data persisted in `sapar-pg-data` volume. |
| `api` | Yes | — | Node.js 20 API. Uploads persisted in `sapar-uploads` volume. |
...
| Volume | Contents |
|---|---|
| `sapar_sapar-pg-data` | All PostgreSQL data |
| `sapar_sapar-uploads` | Invoice logos, expense receipt attachments, company logos |
| `sapar_sapar-redis-data` | Redis persistence (redis profile only) |
| `sapar_sapar-mongo-data` | MongoDB data (mongo profile only) |
