...
All third-party integrations in Sapar follow a BYOK (Bring Your Own Key)
...
Sapar sends email for invoice delivery, payment reminders, quotations,
...
SMTP_FROM=Sapar <billing@yourcompany.com>
```
...
3. In Sapar, go to **Settings → Payment Gateways → Razorpay**.
...
Razorpay sends payment events to your Sapar instance. The webhook endpoint
is:
...
the generated webhook secret into the Sapar Payment Gateway settings as
**Webhook Secret**. Sapar verifies every inbound webhook using HMAC-SHA256
against this secret.
...
3. In Sapar, go to **Settings → Payment Gateways → Stripe**.
...
Copy the generated signing secret into Sapar's Stripe settings as
**Webhook Secret**. Sapar verifies inbound events using
...
Sapar includes AI-powered document extraction (parse receipts and invoices)
...
Sapar includes a bridge for an external WhatsApp CRM service. This is an
optional integration for businesses that manage customer communication through
a separate WhatsApp platform.

### What it does

- **SSO exchange**: The WhatsApp CRM can authenticate users into Sapar using
  a signed JWT (HMAC). On success, Sapar provisions or updates the user
  account.
- **Customer sync**: The CRM can push customer/contact records into Sapar
...
`sapar_api_key` setting. All inbound server-to-server requests from the
