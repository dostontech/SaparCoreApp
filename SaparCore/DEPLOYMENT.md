# Sapar — Production Deployment Guide

This guide deploys the full Sapar stack (API + web + PostgreSQL) with Docker
...
- The three project folders kept **side by side** (the compose build contexts are
  ...
  ├── sapar-typescript-backend/
  └── sapar-typescript-frontend/
  ```
...
| `DATABASE_URL` | `postgresql://sapar:<password>@postgres:5432/sapar?schema=public` | host stays `postgres` (the compose service name) |
...
- **Postgres data** — volume `sapar-pg-data`. Logical dump:
  ```bash
  docker compose --env-file docker/.env -f docker/docker-compose.yml \
    exec -T postgres pg_dump -U sapar sapar | gzip > sapar-$(date +%F).sql.gz
  ```
  Restore: `gunzip -c backup.sql.gz | docker compose ... exec -T postgres psql -U sapar sapar`.
- **Uploaded files** — volume `sapar-uploads` (invoice logos, attachments, etc.):
  ```bash
  docker run --rm -v sapar_sapar-uploads:/data -v "$PWD":/backup alpine \
...
| Open a DB shell | `docker compose --env-file docker/.env -f docker/docker-compose.yml exec postgres psql -U sapar sapar` |
...
- [ ] `CORS_ORIGIN` set to your production domain (e.g. `https://app.sapar.uz`).
      Without this, all API calls from the browser will be blocked by CORS in production.
...
- [ ] Security headers confirmed: run `curl -I https://app.yourdomain.com/api/healthz`
      and verify `X-Frame-Options`, `X-Content-Type-Options`, and `Strict-Transport-Security` are present.
