...
export class SaparEdiHubService {
  ...
  static async signAndDispatch(params: {
    ...
    if (payload.buyer.tin && payload.buyer.tin !== '000000000') {
      const allCompanies = await prisma.companySettings.findMany();
      const matchingBuyerCompany = allCompanies.find(
        (c: any) => c.companyTaxNumber === payload.buyer.tin || c.taxNumber === payload.buyer.tin
      );
