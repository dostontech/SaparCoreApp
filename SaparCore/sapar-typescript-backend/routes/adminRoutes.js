...
// Note: Legacy foreign payment gateways (Razorpay/Stripe) removed per AGENTS.md Central Asia scope.
...
const soliqTaxReportsController = require('../controllers/soliqTaxReportsController');
...
router.post('/demo/seed-data', protect, /* self */ demoSeederController.seedCurrentUserData);
router.get('/demo/seed-data', protect, /* self */ demoSeederController.seedCurrentUserData);
...
// Legacy Foreign Gateways (Razorpay & Stripe) removed per AGENTS.md regional scope (Uzbekistan & Central Asia use Payme/Click)
...
router.get('/dashboard/sales-dynamics', protect, requirePermission('dashboard', 'view'), dashboardController.getSalesDynamics);
...
router.post('/journal-entries/:id/reverse', protect, requirePermission('journal-entries', 'create'), journalEntryController.reverseEntry);
router.delete('/journal-entries/:id', protect, requirePermission('journal-entries', 'delete'), journalEntryController.remove);

// Contras (O'zaro hisob-kitoblar / Взаимозачет)
router.get('/contras', protect, requirePermission('journal-entries', 'view'), contraController.listContras);
router.get('/contras/contacts-summary', protect, requirePermission('journal-entries', 'view'), contraController.getEligibleContacts);
router.get('/contras/contact-details/:contactId', protect, requirePermission('journal-entries', 'view'), contraController.getContactContraDetails);
router.post('/contras', protect, requirePermission('journal-entries', 'create'), contraController.createContra);
...
// Uzbekistan State Tax Committee (Soliq) Declarations
router.get('/reports/soliq-qqs', protect, requirePermission('accounting-reports', 'view'), soliqTaxReportsController.getSoliqQqsDeclaration);
router.get('/reports/soliq-jshods', protect, requirePermission('accounting-reports', 'view'), soliqTaxReportsController.getSoliqJshodsDeclaration);
router.get('/reports/soliq-aylanma', protect, requirePermission('accounting-reports', 'view'), soliqTaxReportsController.getSoliqAylanmaDeclaration);
router.post('/reports/soliq-submit', protect, requirePermission('accounting-reports', 'create'), soliqTaxReportsController.submitSoliqDeclaration);
...
// 🏛️ SAPAR In-House Soliq EDI & E-Faktura Hub
router.get('/e-invoices', protect, requirePermission('invoices', 'view'), eInvoiceController.list);
router.get('/e-invoices/prepare/:invoiceId', protect, requirePermission('invoices', 'view'), eInvoiceController.prepareSoliqDocument);
router.post('/e-invoices/send-signed', protect, requirePermission('invoices', 'create'), eInvoiceController.submitSignedDocument);
router.get('/e-invoices/status/:docId', protect, requirePermission('invoices', 'view'), eInvoiceController.checkStatus);
router.get('/e-invoices/inbox', protect, requirePermission('invoices', 'view'), eInvoiceController.getInbox);
router.get('/e-invoices/:invoiceId/export-soliq-xml', protect, requirePermission('invoices', 'view'), eInvoiceController.exportSoliqXml);

// In-House E-Hujjatlar (Electronic Documents & Two-Way E-IMZO Signing)
router.get('/e-documents', protect, requirePermission('e-documents', 'view'), eDocumentController.listEDocuments);
router.get('/e-documents/:id', protect, requirePermission('e-documents', 'view'), eDocumentController.getEDocumentById);
router.post('/e-documents', protect, requirePermission('e-documents', 'create'), eDocumentController.createEDocument);
router.delete('/e-documents/:id', protect, requirePermission('e-documents', 'delete'), eDocumentController.deleteEDocument);
router.post('/e-documents/generate/akt-sverki', protect, requirePermission('e-documents', 'create'), eDocumentController.generateAktSverki);
router.post('/e-documents/generate/empowerment', protect, requirePermission('e-documents', 'create'), eDocumentController.generateEmpowerment);
router.post('/e-documents/generate/contract', protect, requirePermission('e-documents', 'create'), eDocumentController.generateContract);
router.post('/e-documents/:id/sign-sender', protect, requirePermission('e-documents', 'edit'), eDocumentController.signAsSender);
router.post('/public/e-documents/:id/sign-recipient', /* self */ eDocumentController.signAsRecipient);
router.post('/public/e-documents/:id/reject', /* self */ eDocumentController.rejectDocument);
router.get('/public/e-documents/:id/verify', /* self */ eDocumentController.verifyDocument);
...
// 🇺🇿 Uzbekistan Automated Payroll & Tabel routes
const tabelController = require('../controllers/tabelController');
router.post('/payroll/calculate-uz', protect, requirePermission('payroll', 'view'), payrollController.calculateUzbekPayroll);
router.get('/payroll/vacation-sick-calc', protect, requirePermission('payroll', 'view'), payrollController.calculateVacationAndSick);
router.get('/payroll/payslip/:lineId', protect, requirePermission('payroll', 'view'), payrollController.getPayslipDocument);
router.get('/payroll/tabel', protect, requirePermission('payroll', 'view'), tabelController.getTabelMatrix);
router.post('/payroll/tabel', protect, requirePermission('payroll', 'edit'), tabelController.saveTabelMatrix);

// 🛒 Point of Sale (POS) & Retail Terminal routes
const posController = require('../controllers/posController');
router.get('/pos/products', protect, requirePermission('pos', 'view'), posController.getPosProducts);
router.post('/pos/checkout', protect, requirePermission('pos', 'create'), posController.posCheckout);
router.get('/pos/shift/current', protect, requirePermission('pos', 'view'), posController.getCurrentShift);
router.post('/pos/shift/open', protect, requirePermission('pos', 'create'), posController.openShift);
router.post('/pos/shift/close', protect, requirePermission('pos', 'edit'), posController.closeShift);
router.get('/pos/shift/x-report', protect, requirePermission('pos', 'view'), posController.getXReport);
router.get('/pos/receipt/:receiptId', protect, requirePermission('pos', 'view'), posController.getPosReceipt);

// 💼 CRM Sales Pipeline & Deals routes
const crmDealsController = require('../controllers/crmDealsController');
router.get('/crm/pipeline', protect, requirePermission('crm', 'view'), crmDealsController.getDealsPipeline);
router.post('/crm/deals', protect, requirePermission('crm', 'create'), crmDealsController.createDeal);
router.put('/crm/deals/:id/stage', protect, requirePermission('crm', 'edit'), crmDealsController.updateDealStage);
router.delete('/crm/deals/:id', protect, requirePermission('crm', 'delete'), crmDealsController.deleteDeal);

// 📁 Project Workspace & Tasks routes
const projectWorkspaceController = require('../controllers/projectWorkspaceController');
router.get('/projects/:projectId/workspace', protect, requirePermission('projects', 'view'), projectWorkspaceController.getProjectWorkspace);
router.post('/projects/:projectId/tasks', protect, requirePermission('projects', 'create'), projectWorkspaceController.createProjectTask);
router.put('/projects/:projectId/tasks/:taskId/stage', protect, requirePermission('projects', 'edit'), projectWorkspaceController.updateProjectTaskStage);
router.delete('/projects/:projectId/tasks/:taskId', protect, requirePermission('projects', 'delete'), projectWorkspaceController.deleteProjectTask);

// 💳 Uzbekistan Payment Gateways & Banking API routes
const uzbekPaymentGatewaysController = require('../controllers/uzbekPaymentGatewaysController');
router.get('/payments/uz-gateways/settings', protect, requirePermission('finance-settings', 'view'), uzbekPaymentGatewaysController.getGatewaySettings);
...
router.get('/payments/uzqr/status/:referenceId', protect, /* self */ uzbekPaymentGatewaysController.checkUzQrPaymentStatus);

// 🏷️ Asl Belgisi Digital Marking & Decree No. 296 Compliance
const markingController = require('../controllers/markingController');
router.get('/marking/categories', protect, requirePermission('inventory', 'view'), markingController.getMarkingCategories);
router.post('/marking/verify', protect, requirePermission('inventory', 'view'), markingController.verifyMarkingCode);
router.post('/marking/register', protect, requirePermission('inventory', 'create'), markingController.registerMarkingCodes);
router.post('/marking/write-off', protect, requirePermission('inventory', 'edit'), markingController.writeOffMarkingCode);
...
router.post('/helpdesk/tickets', protect, requirePermission('helpdesk', 'create'), helpdeskController.createTicket);
router.post('/helpdesk/tickets/:ticketId/reply', protect, requirePermission('helpdesk', 'create'), helpdeskController.replyToTicket);
router.put('/helpdesk/tickets/:ticketId/status', protect, requirePermission('helpdesk', 'edit'), helpdeskController.updateTicketStatus);

// 🔑 Tenant Subscription & Feature Provisioning routes
const subscriptionProvisioningController = require('../controllers/subscriptionProvisioningController');
router.get('/subscriptions/current', protect, requirePermission('settings', 'view'), subscriptionProvisioningController.getCompanySubscription);
router.post('/subscriptions/update-plan', protect, requirePermission('settings', 'edit'), subscriptionProvisioningController.updateCompanyPlan);

// 🏛️ Uzbekistan National Accounting Standard (21-son BHMS) & Financial Statements
const bhmsAccountingController = require('../controllers/bhmsAccountingController');
router.get('/accounting/bhms/chart-of-accounts', protect, requirePermission('accounting', 'view'), bhmsAccountingController.getBhmsChartOfAccounts);
...
// SaaS Super-Admin & Client Management + Onboarding
...
router.get('/tabel/matrix', protect, requirePermission('employees', 'view'), tabelController.getTabelMatrix);
router.post('/tabel/matrix', protect, requirePermission('employees', 'edit'), tabelController.saveTabelMatrix);
router.post('/tabel/save', protect, requirePermission('employees', 'edit'), tabelController.saveTabelMatrix);

// Route aliases for frontend and API consistency
router.get('/crm/deals/pipeline', protect, requirePermission('crm', 'view'), crmDealsController.getDealsPipeline);
router.get('/crm/deals', protect, requirePermission('crm', 'view'), crmDealsController.getDealsPipeline);
router.get('/crm-deals/pipeline', protect, requirePermission('crm', 'view'), crmDealsController.getDealsPipeline);
router.get('/crm-deals', protect, requirePermission('crm', 'view'), crmDealsController.getDealsPipeline);
router.get('/projects/workspace/:projectId', protect, requirePermission('projects', 'view'), projectWorkspaceController.getProjectWorkspace);
router.get('/warehouses', protect, requirePermission('inventory', 'view'), inventoryController.listInventory);
router.get('/warehouse', protect, requirePermission('inventory', 'view'), inventoryController.listInventory);
router.get('/debit-notes', protect, requirePermission('debit-notes', 'view'), debitNoteController.getAllDebitNotes);
router.get('/delivery-challans', protect, requirePermission('delivery-challans', 'view'), deliveryChallanController.getDeliveryChallans);
router.get('/category', protect, requirePermission('product-services', 'view'), CategoryController.getAllCategories);
router.get('/brand', protect, requirePermission('product-services', 'view'), BrandsController.getAllBrands);

// User & Staff Management routes
router.get('/staff', protect, requirePermission('users', 'view'), userController.listStaffUsers);
router.post('/staff', protect, requirePermission('users', 'create'), userController.createStaffUser);
router.put('/staff/:id', protect, requirePermission('users', 'edit'), userController.updateStaffUser);
router.delete('/staff/:id', protect, requirePermission('users', 'delete'), userController.deleteStaffUser);
router.get('/users', protect, requirePermission('users', 'view'), userController.listStaffUsers);
router.post('/users', protect, requirePermission('users', 'create'), userController.createStaffUser);
router.put('/users/:id', protect, requirePermission('users', 'edit'), userController.updateStaffUser);
router.delete('/users/:id', protect, requirePermission('users', 'delete'), userController.deleteStaffUser);

// Roles & Permissions routes
router.get('/roles', protect, requirePermission('roles-permission', 'view'), roleController.getRoles);
router.post('/roles', protect, requirePermission('roles-permission', 'create'), roleController.createRole);
router.put('/roles/:id', protect, requirePermission('roles-permission', 'edit'), roleController.updateRole);
router.delete('/roles/:id', protect, requirePermission('roles-permission', 'delete'), roleController.deleteRole);

router.get('/permissions/:roleId', protect, requirePermission('roles-permission', 'view'), permissionController.getPermissionsByRole);
router.post('/permissions', protect, requirePermission('roles-permission', 'edit'), permissionController.createOrUpdatePermissions);
router.get('/modules/hierarchy', protect, requirePermission('roles-permission', 'view'), permissionController.getModuleHierarchy);
