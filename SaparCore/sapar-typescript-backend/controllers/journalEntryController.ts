...
export async function reverseEntry(req: Request, res: Response): Promise<void> {
  try {
    const userId = requireUserId(req);
    const { id } = req.params as { id: string };
    ...
    const reversal = await prisma.journalEntry.create({
      data: {
        ...
        entryNumber,
        entryDate: new Date(),
        reference: `STORNO-${original.entryNumber || original.reference || id.slice(0, 6)}`,
        description: `Storno (Teskari provodka / Reversal): ${original.description || original.entryNumber}`,
        lines: {
    ...
    res.status(201).json({
      success: true,
      message: 'Journal entry reversed (Storno created)',
      data: { journalEntry: reversal },
    });
  } catch (err) {
    if (err instanceof UnauthorizedError) {
      res.status(401).json({ success: false, message: err.message });
    ...
    console.error('journalEntry reverseEntry error:', err);
    res.status(500).json({ success: false, message: 'Failed to reverse journal entry' });
  }
}

const handlers = { list, getById, create, remove, reverseEntry };
...
module.exports.reverseEntry = reverseEntry;
