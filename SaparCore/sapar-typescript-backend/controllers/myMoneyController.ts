...
export async function getMyMoney(req: Request, res: Response): Promise<void> {
  try {
    ...
    const allTxnsRaw = prisma.bankTransaction && typeof prisma.bankTransaction.findMany === 'function'
    ...
    const expenseFinder = (prisma as any).expense?.findMany?.bind((prisma as any).expense);
    const reimbursableExpensesRaw = expenseFinder ? await expenseFinder({
      where: {
        isDeleted: false,
    ...
    const payRunLineFinder = (prisma as any).payRunLine?.findMany?.bind((prisma as any).payRunLine);
    const runLineRowsRaw = payRunLineFinder ? await payRunLineFinder({
      where: {
    ...
    const runLineRows = ((runLineRowsRaw || []) as { net: Prisma.Decimal; payRun: { periodEnd: Date; taxMonth: number; taxYearLabel: string } }[])
      .filter((l) => l.payRun && l.payRun.periodEnd >= taxYear.start && l.payRun.periodEnd <= taxYear.end)
      .map((l) => ({
