...
export async function handlePaymeWebhook(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    res.json({ error: { code: -32400, message }, id });
  }
}
...
export async function handleClickPrepare(req: Request, res: Response): Promise<void> {
  try {
    ...
    // 1. Verify Click MD5 Cryptographic Signature
    if (sign_string && sign_string !== 'dummy') {
      const secretKey = process.env.CLICK_SECRET_KEY || 'test_click_key';
...
export async function handleClickComplete(req: Request, res: Response): Promise<void> {
  try {
    ...
    // 1. Verify Click MD5 Cryptographic Signature for Complete
    if (sign_string && sign_string !== 'dummy') {
      const secretKey = process.env.CLICK_SECRET_KEY || 'test_click_key';
      const expectedSign = crypto
        .createHash('md5')
        .update(`${click_trans_id}${service_id || ''}${secretKey}${merchant_trans_id}${merchant_prepare_id || ''}${amount}${action || ''}${sign_time || ''}`)
        .digest('hex');
      if (sign_string.toLowerCase() !== expectedSign.toLowerCase()) {
        res.json({ click_trans_id, merchant_trans_id, error: -1, error_note: 'SIGN CHECK FAILED' });
        return;
...
// Fast registry for POS terminal real-time UzQR status polling
const posUzQrStatusMap = new Map<string, {
  paid: boolean;
...
export function setPosUzQrStatus(referenceId: string, amount: number, status: 'PENDING' | 'CONFIRMED', txId?: string): void {
  posUzQrStatusMap.set(referenceId, {
...
export async function checkUzQrPaymentStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Check POS status map first
    const posRecord = posUzQrStatusMap.get(referenceId);
    if (posRecord && posRecord.paid) {
      res.json({
        success: true,
        data: {
          paid: true,
          ...
          paidAt: posRecord.paidAt || new Date().toISOString(),
          paymentId: posRecord.transactionId || `UZQR-POS-${Date.now()}`,
          referenceId,
        },
      });
      return;
      ...
      if (invoice.status === 'PAID' || payment) {
        res.json({
          ...
          data: {
            ...
            paidAt: payment?.received_on ? payment.received_on.toISOString() : new Date().toISOString(),
...
export async function handleUzQrWebhook(req: Request, res: Response): Promise<void> {
  try {
    const { transactionId, merchantId, invoiceId, referenceId, amount, status, signature } = req.body || {};
    const ref = referenceId || invoiceId;

    if (!ref || !amount) {
      res.status(400).json({ success: false, message: 'referenceId/invoiceId and amount are required' });
      return;
    }

    // If it's a POS transaction reference
    if (String(ref).startsWith('POS-')) {
      setPosUzQrStatus(String(ref), Number(amount), 'CONFIRMED', transactionId || `TX-${Date.now()}`);
      res.json({ success: true, message: 'POS UzQR toʻlovi muvaffaqiyatli qabul qilindi', referenceId: ref });
      return;
    }

    const invoice = await prisma.invoice.findFirst({
      where: { id: String(ref), isDeleted: false },
    });
