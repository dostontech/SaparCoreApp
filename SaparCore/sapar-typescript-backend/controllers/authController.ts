...
export async function register(req: Request, res: Response): Promise<void> {
  ...
  const { firstName, lastName, email, phone, password, companyName, subdomain } = req.body as {
    firstName?: string;
    lastName?: string;
    email: string;
    phone?: string;
  ...
  try {
    ...
    // Ensure the Owner role exists and link it to the new company admin user
      ...
      try {
        await prisma.companySettings.create({
          data: {
            ...
            publicBaseUrl: subdomain ? `https://${subdomain.toLowerCase().trim()}.app.sapar.uz` : null,
          },
        });
      } catch (compErr) {
