...
// ---------------------------------------------------------------------------
// 🇺🇿 Uzbekistan Native Calculation Handlers
// ---------------------------------------------------------------------------

import {
  calculateUzbekSalary,
  calculateVacationPay,
  calculateSickLeavePay,
  type SalaryInput,
} from '../lib/payroll/uzbekPayrollEngine';

export async function calculateUzbekPayroll(req: Request, res: Response): Promise<void> {
  try {
    const input = req.body as SalaryInput;
    const result = calculateUzbekSalary(input);
...
export async function calculateVacationAndSick(req: Request, res: Response): Promise<void> {
  try {
    const type = req.query.type as string; // 'vacation' | 'sick'
    if (type === 'sick') {
      ...
      const result = calculateVacationPay(total12Month, days);
      res.json({ success: true, data: result });
    }
  } catch (err) {
    res.status(500).json({ success: false, message: (err as Error).message });
...
export async function getPayslipDocument(req: Request, res: Response): Promise<void> {
  try {
    const userId = requireUserId(req);
    const { lineId } = req.params;

    // Fetch line details
    const line = await (prisma as any).payRunLine.findFirst({
      where: { id: lineId },
      include: {
        payRun: true,
        employeeUser: {
    ...
    const calc = calculateUzbekSalary({ baseSalary: gross });

    const payslip = {
      payslipNumber: `PAY-${line?.id?.substring(0, 8).toUpperCase() || '2026-01'}`,
      period: line?.payRun?.payPeriod || '2026-08 (Avgust 2026)',
      taxYear: line?.payRun?.taxYearLabel || '2026-yil',
      generatedDate: new Date().toISOString().substring(0, 10),
      company: {
        name: company?.companyName || 'SAPAR TECHNOLOGIES MCHJ',
        tin: (company as any)?.companyTaxNumber || (company as any)?.taxNumber || '308765432',
        address: company?.address || 'Toshkent shahri',
      },
      employee: {
        name: `${line?.employeeUser?.firstName || ''} ${line?.employeeUser?.lastName || ''}`.trim() || 'Xodim',
        email: line?.employeeUser?.email || '',
        phone: line?.employeeUser?.phone || '',
        position: 'Mutaxassis / Xodim',
      },
      calculation: calc,
    };
