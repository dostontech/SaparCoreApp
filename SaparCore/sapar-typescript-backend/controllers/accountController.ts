...
export async function update(req: Request, res: Response): Promise<void> {
  try {
    ...
    const existing = await prisma.account.findFirst({
      where: { id, userId, isDeleted: false },
      include: {
        _count: { select: { journalLines: true, roleMappings: true } },
      },
    });
    if (!existing) {
      res.status(404).json({ success: false, message: 'Account not found' });
    ...
    if (body.accountType !== undefined && body.accountType !== existing.accountType) {
      if (existing.roleProtected) {
        res.status(400).json({ success: false, message: 'Tizim tomonidan himoyalangan hisob turini (accountType) oʻzgartirish mumkin emas' });
        return;
      }
      if (existing._count.journalLines > 0) {
        res.status(400).json({ success: false, message: 'Tranzaksiyalar mavjud boʻlgan hisob turini oʻzgartirish mumkin emas' });
        return;
      }
    }
...
export async function remove(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (existing.roleProtected) {
      res.status(400).json({ success: false, message: 'Tizim boshqaruv hisoblarini (control accounts) oʻchirib boʻlmaydi' });
      return;
    }

    if (existing._count.roleMappings > 0) {
      res.status(400).json({ success: false, message: 'Tizim rollariga biriktirilgan hisobni oʻchirib boʻlmaydi' });
      return;
    }

    if (existing._count.journalLines > 0) {
      res.status(400).json({ success: false, message: 'Bosh kitobda tranzaksiya tarixi mavjud boʻlgan hisobni oʻchirib boʻlmaydi' });
      return;
    }

    if (existing._count.children > 0) {
      res.status(400).json({ success: false, message: 'Faol quyi (sub-account) hisoblari mavjud boʻlgan hisobni oʻchirib boʻlmaydi' });
      return;
    }
