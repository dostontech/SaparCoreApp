...
// - ssoExchange:  verify a whatsappcrm-issued HMAC JWT, upsert a sapar User
//                 by email, mint a sapar session token. Called by the
//                 sapar frontend's /sso landing route.
...
// the source of truth for contacts. Sapar stays standalone for everything
// else.
...
exports.ssoExchange = async (req, res) => {
  ...
  if (!secret) {
    return res.status(503).json({
      success: false,
      message: 'SSO not configured on this sapar instance.',
    });
  }
  ...
  try {
    ...
    if (!user) {
      // First-time SSO from whatsappcrm — provision a sapar user.
      ...
      // password through sapar's normal flow.
...
const resolveOwnerUserId = async () => {
  // V1 design: a single admin owns externally-synced customers. Sapar's
...
exports.upsertCustomer = async (req, res) => {
  ...
  if (!ownerId) {
    return res.status(409).json({
      success: false,
      message: 'No admin user configured on sapar yet — register an admin first.',
    });
  }

  // Sapar Customer requires a unique email per user. whatsappcrm contacts
  ...
  try {
    ...
    // duplicate-key path: a sapar-native customer already owns this email.
    // Bail with a 409 so whatsappcrm logs it without retrying forever.
    if (err && err.code === 'P2002') {
      return res.status(409).json({
        success: false,
        message: 'A sapar customer with this email already exists outside the integration.',
