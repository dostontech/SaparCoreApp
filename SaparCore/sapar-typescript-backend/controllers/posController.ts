  ...
  try {
    ...
    for (const item of items) {
      ...
      for (const code of markingCodesToCheck) {
        const val = await validateMarkingCodeForPosAsync(userId, code, { id: item.id, name: item.name });
        if (val.blocked) {
          res.status(422).json({
            success: false,
            blocked: true,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Create PosReceipt
      const qrCodeUrl = `https://soliq.uz/check?tin=${(company as any)?.companyTaxNumber || (company as any)?.taxNumber || '308765432'}&fiscal=${receiptNumber}`;
    ...
    // Record used marking codes as SOLD (persisted to PostgreSQL + memory)
    for (const item of items) {
      const anyItem = item as any;
      const markingCodesToCheck: string[] = [];
      if (anyItem.markingCode) markingCodesToCheck.push(anyItem.markingCode);
      if (Array.isArray(anyItem.markingCodes)) markingCodesToCheck.push(...anyItem.markingCodes);
      for (const code of markingCodesToCheck) {
        await recordMarkingCodeSoldAsync(userId, code, (result as any)?.receiptId, item.id);
      }
    }
