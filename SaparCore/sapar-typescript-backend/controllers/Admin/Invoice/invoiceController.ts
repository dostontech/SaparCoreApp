...
function normaliseItems(raw: unknown): IncomingItem[] {
  ...
  return (parsedRaw as IncomingItem[]).map((item) => {
    const rawAny = item as Record<string, unknown>;
    const qtyVal = asNumber(item.qty ?? rawAny.quantity, 0);
    const rateVal = asNumber(item.rate, 0);
    const taxRateVal = asNumber(item.taxRate ?? rawAny.taxPercent, 0);
    return {
      ...
      tax: asNumber(item.tax ?? rawAny.taxAmount, 0),
      taxRate: taxRateVal > 0 ? taxRateVal : undefined,
      tax_group_id: item.tax_group_id,
      amount: asNumber(item.amount, rateVal * qtyVal),
      discount_type: item.discount_type ?? (rawAny.discountType as string | undefined),
      ...
      taxes: Array.isArray(item.taxes) && item.taxes.length > 0
        ...
        : (taxRateVal > 0 ? [{ name: 'Tax', percent: taxRateVal }] : undefined),
    ...
    const invoice = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.invoice.create({
        data: {
          ...
          billToContactId: quotation.billToContactId ?? quotation.contactId ?? null,
          currencyCode: quotation.currencyCode ?? null,
          exchangeRate: (quotation as any).exchangeRate ?? null,
          taxTreatment: quotation.taxTreatment ?? null,
          ...
          billTo: quotation.customerId ? quotation.billTo : null,
