...
async function applyPurchaseReceiptEffects(
  ...
  if (status !== 'new' && status !== 'cancelled') {
    const fxMultiplier = created.exchangeRate != null ? Number(created.exchangeRate) : 1;
    const effectiveFx = fxMultiplier > 0 ? fxMultiplier : 1;
    const inventoryLineParams = items.map((item) => ({
      amount: asNumber(item.amount, asNumber(item.rate, 0) * asNumber(item.qty, 0)) * effectiveFx,
      qty: asNumber(item.qty, 0),
    }));
    ...
    for (let itemIdx = 0; itemIdx < items.length; itemIdx++) {
      ...
      const baseUnitCost = asNumber(item.rate, 0) * effectiveFx;
      // P3.5: landed-inclusive unit cost = base rate (in functional currency) + landed addition per unit.
...
export async function createPurchase(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const billFrom = (body.billFrom as string) || authUserId;
...
export async function updatePurchase(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (status !== 'new' && status !== 'cancelled') {
        const fxMultiplier = upd.exchangeRate != null ? Number(upd.exchangeRate) : 1;
        const effectiveFx = fxMultiplier > 0 ? fxMultiplier : 1;
        ...
        for (let itemIdx = 0; itemIdx < items.length; itemIdx++) {
          ...
          const qty = asNumber(item.qty, 0);
          if (!qty) continue;
          const landedUnitCost = asNumber(item.rate, 0) * effectiveFx + (landedAdditions[itemIdx] ?? 0);
          await applyStockAdjustment(tx as unknown as Parameters<typeof applyStockAdjustment>[0], {
            productId,
            userId: requireUserId(req),
            qtyDelta: qty,
