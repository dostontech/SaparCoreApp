...
export async function createFixedAsset(req: Request, res: Response): Promise<void> {
  try {
    ...
    const salvageStr = salvageValue ? String(salvageValue) : '0';
    ...
    const asset = await prisma.$transaction(async (tx) => {
      const created = await tx.fixedAsset.create({
        data: {
          userId,
          name,
          cost: new Prisma.Decimal(costStr),
          salvageValue: new Prisma.Decimal(salvageStr),
      ...
      if (postAcquisition) {
        ...
        await postAssetAcquisition(
          tx as never,
          { userId, assetId: created.id, date: acqDate, cost: costStr },
        );
      }
