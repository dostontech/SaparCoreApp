...
const checkMailHealth = async () => {
  try {
    if (process.env.DEMO_MODE === 'true') {
      return { configured: true, status: 'demo_mock', provider: 'demo' };
    }
    const settings = await loadActiveSettings();
    const { transport } = buildTransport(settings);
    if (typeof transport.verify === 'function') {
      await transport.verify();
      return { configured: true, status: 'verified', provider: settings?.resend_status ? 'resend' : settings?.smtp_status ? 'smtp' : 'env' };
    }
    return { configured: true, status: 'ready', provider: 'custom' };
  } catch (err) {
    return { configured: false, status: 'unconfigured_or_error', message: err.message, code: err.code };
  }
};
...
module.exports = { sendMail, clearMailerCache, checkMailHealth };
