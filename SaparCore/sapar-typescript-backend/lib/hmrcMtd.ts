...
const VENDOR_PRODUCT_NAME = 'Sapar';
...
export interface FraudContext {
  ...
  deviceId?: string;
  /** The originating end-user id within Sapar — Gov-Client-User-IDs. */
  userId?: string;
...
export function fraudHeaders(ctx: FraudContext): Record<string, string> {
  ...
  if (ctx.userId) headers['Gov-Client-User-IDs'] = `sapar=${encodeURIComponent(ctx.userId)}`;
