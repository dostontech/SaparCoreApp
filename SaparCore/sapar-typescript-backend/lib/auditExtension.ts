      ...
      $allModels: {
        async $allOperations({ model, operation, args, query }) {
          ...
          if (needsBefore && (args as any)?.where) {
            try {
              let whereClause = (args as any).where;
              const keys = Object.keys(whereClause || {});
              if (keys.length === 1 && typeof whereClause[keys[0]] === 'object' && whereClause[keys[0]] !== null && !Array.isArray(whereClause[keys[0]])) {
                whereClause = { ...whereClause[keys[0]] };
              }
