...
/**
 * Async validation checking PostgreSQL first, with cache fallback.
 */
export async function validateMarkingCodeForPosAsync(
  tenantId: string,
  rawCode: string,
  productContext?: { id?: string; name?: string; requiresExpiration?: boolean }
): Promise<MarkingValidationResult> {
  const syncResult = validateMarkingCodeForPos(tenantId, rawCode, productContext);
  if (syncResult.blocked) return syncResult;

  try {
    const dbRecord = await prisma.markingCode.findUnique({
      where: { userId_rawCode: { userId: tenantId, rawCode: rawCode.trim() } },
    });
    ...
    if (dbRecord) {
      if (dbRecord.status === 'SOLD') {
        return {
          ...
          blocked: true,
          reason: 'ALREADY_SOLD',
          message: `Ushbu markirovka kodi avval sotilgan (${dbRecord.soldAt?.toISOString() || 'Oldingi chekda'}). Takroriy sotish taqiqlanadi!`,
          parsed: syncResult.parsed,
        };
      }
      if (dbRecord.status === 'WRITTEN_OFF') {
        return {
          valid: false,
          blocked: true,
          reason: 'WRITTEN_OFF',
          message: `Ushbu tovar avval hisobdan chiqarilgan (${dbRecord.writeOffReason || '9430-hisob'}). Sotish mumkin emas!`,
          parsed: syncResult.parsed,
        };
      }
    }
  } catch (dbErr) {
    console.warn('DB MarkingCode lookup fallback to memory:', dbErr);
  }
...
/**
 * Persists a SOLD marking code to PostgreSQL and memory store.
 */
export async function recordMarkingCodeSoldAsync(
  tenantId: string,
  rawCode: string,
  posReceiptId?: string,
  productId?: string
): Promise<void> {
  recordMarkingCodeSold(tenantId, rawCode, posReceiptId, productId);
  const parsed = parseAslBelgisiDataMatrix(rawCode);

  try {
    await prisma.markingCode.upsert({
      where: { userId_rawCode: { userId: tenantId, rawCode: rawCode.trim() } },
      create: {
        userId: tenantId,
    ...
    console.warn('Marking code sold DB persistence error:', err);
  }
}

/**
 * Registers an inbound marking code into PostgreSQL and memory store.
 */
export async function registerInboundMarkingCodeAsync(
  tenantId: string,
  rawCode: string,
  productId?: string,
  customExpiration?: string
): Promise<StoredMarkingRecord> {
  const memRecord = registerInboundMarkingCode(tenantId, rawCode, productId, customExpiration);
  const parsed = parseAslBelgisiDataMatrix(rawCode);
  ...
  try {
    await prisma.markingCode.upsert({
      ...
      update: {
        ...
        expirationDate: customExpiration || parsed.expirationDate || undefined,
        status: 'ACTIVE',
      },
    });
  } catch (err) {
    console.warn('Marking code inbound DB registration error:', err);
  }
...
/**
 * Writes off an expired marked code to GL Account 9430 with DB persistence.
 */
export async function writeOffExpiredMarkingCodeAsync(
  tenantId: string,
  rawCode: string,
  reason: string = 'Muddati oʻtgan tovar (VM 296-son qaror)'
): Promise<{ success: boolean; record?: StoredMarkingRecord; glAccount: string }> {
  const result = writeOffExpiredMarkingCode(tenantId, rawCode, reason);
  const parsed = parseAslBelgisiDataMatrix(rawCode);
  ...
  try {
    ...
    console.warn('Marking code writeoff DB persistence error:', err);
  }
