...
async function main() {
  ...
  // =========================================================================
  // 3. Soliq Aylanmadan Olinadigan Soliq 4% (Form 10104_18) Standard
  // =========================================================================
  console.log('--- 3. Testing Soliq Turnover Tax 4% Standard Declaration (Form 10104_18) ---');
  ...
  if (aylanmaRes.status === 200 && aylanmaData?.formCode === '10104_18' && aylanmaData?.taxRatePercent === 4) {
    console.log('✓ Form 10104_18 (4% Standard) PASS: Turnover tax 4% declaration generated cleanly.\n');
  } else {
    console.log('❌ Form 10104_18 (4% Standard) FAIL: Turnover tax generation mismatch.\n');
  }

  // =========================================================================
  // 3b. Soliq Aylanmadan Olinadigan Soliq 1% (Form 10104_18) Individual Entrepreneur (YaTT)
  // =========================================================================
  console.log('--- 3b. Testing Soliq Turnover Tax 1% IE/YaTT Concession (Form 10104_18) ---');
  const aylanmaIeRes = await axios.get(`${API_BASE}/admin/reports/soliq-aylanma?rate=1`, { headers });
  const aylanmaIeData = aylanmaIeRes.data.data;

  console.log(`1. IE Turnover Tax Form Code: ${aylanmaIeData?.formCode} | Tax: ${aylanmaIeData?.taxType}`);
  console.log(`2. Gross Revenue = ${Number(aylanmaIeData?.summary?.grossRevenue).toLocaleString()} UZS | Rate = ${aylanmaIeData?.taxRatePercent}% | Soliq = ${Number(aylanmaIeData?.summary?.calculatedTaxSum).toLocaleString()} UZS`);
  console.log(`3. Is IE Flag: ${aylanmaIeData?.isIndividualEntrepreneur}`);

  if (
    aylanmaIeRes.status === 200 &&
    aylanmaIeData?.formCode === '10104_18' &&
    aylanmaIeData?.taxRatePercent === 1 &&
    aylanmaIeData?.isIndividualEntrepreneur === true &&
    aylanmaIeData?.summary?.calculatedTaxSum === (aylanmaIeData?.summary?.grossRevenue * 0.01)
  ) {
    console.log('✓ Form 10104_18 (1% IE Concession) PASS: Individual Entrepreneur 1% tax calculation verified.\n');
  } else {
    console.log('❌ Form 10104_18 (1% IE Concession) FAIL: Individual Entrepreneur turnover tax calculation mismatch.\n');
  }
