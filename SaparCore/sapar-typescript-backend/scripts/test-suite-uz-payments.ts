...
async function main() {
  ...
  const testInvoiceId = 'inv-test-uz-gateway-001';
  const linksRes = await axios.get(
    `${API_BASE}/admin/payments/uz-gateways/invoice-links/${testInvoiceId}?amount=1200000`,
    { headers }
  );
...
КонецДокумента`;
  ...
  if (importRes.status === 200 && impData?.importedCount > 0 && impData?.totalIncome === 45000000) {
