...
async function main() {
  ...
  const user = await p.user.findFirst({ where: { email: 'admin@demo.sapar.local' } }) || await p.user.findFirst({ where: { user_type: 1 } });
  ...
  await p.companySettings.upsert({
    ...
    create: {
      userId: user.id,
      companyName: 'SAPAR Enterprise LLC',
      email: user.email || 'admin@sapar.uz',
      phone: '+998901234567',
      ...
      goLiveDate: new Date('2020-01-01'),
  ...
  const pmt1Res = await axios.post(
    `${API_BASE}/admin/invoice/payment`,
    {
  ...
  const voidRes = await axios.post(
    `${API_BASE}/admin/invoices/payments/${lastPayment.id}/void`,
  ...
  const convRes = await axios.post(
    `${API_BASE}/admin/quotation-convert-to-invoice/${quotation.id}`,
    {},
    { headers }
  );
  ...
  const linkRes = await axios.post(
    `${API_BASE}/admin/invoices/${createdInv.id}/enable-public-link`,
    {},
    { headers }
  );

  const publicToken = linkRes.data.data?.publicViewToken;
  console.log(`1. Enabled Public Link: Token=${publicToken}`);

  // Test Public Retrieval (without auth header)
  const pubViewRes = await axios.get(`${API_BASE}/public/invoices/${publicToken}`);
